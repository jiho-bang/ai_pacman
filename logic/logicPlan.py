...
def sentence3() -> Expr:
    ...
    """
    alive0 = logic.PropSymbolExpr('PacmanAlive_0')
    alive1 = logic.PropSymbolExpr('PacmanAlive_1')
    born0 = logic.PropSymbolExpr('PacmanBorn_0')
    killed0 = logic.PropSymbolExpr('PacmanKilled_0')
    first = alive1 % ((alive0 & ~killed0) | (~alive0 & born0))
    second = ~(alive0 & born0)
    third = born0
    return logic.conjoin(first, second, third)
    util.raiseNotDefined()
...
def findModelUnderstandingCheck() -> Dict[Expr, bool]:
    ...
    a = Expr('A')
    class lowerCase:
        def __init__(self, arg: str = 'A'):
            self.arg = arg
        def __repr__(self):
            return self.arg
    return {lowerCase('a'): True}
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    combined = list(itertools.combinations(literals, 2))
    conj = [~x | ~y for x, y in combined]
    return conjoin(conj)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    first = PropSymbolExpr(pacman_str, x, y, time = now)
    second = disjoin(possible_causes)
    return first % second
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    pacphysics_sentences.append(exactlyOne(locations))

    directions = []
    for d in DIRECTIONS:
        directions.append(PropSymbolExpr(d, time = t))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        lst = []
        walls = []
        g = PropSymbolExpr(pacman_str, xg, yg, time = t)
        for x, y in non_wall_coords:
            walls.append(PropSymbolExpr(pacman_str, x, y, time = t))
        KB.append(exactlyOne(walls))
        ...
        lst.append(PropSymbolExpr(actions[0], time = t))
        lst.append(PropSymbolExpr(actions[1], time = t))
        lst.append(PropSymbolExpr(actions[2], time = t))
        lst.append(PropSymbolExpr(actions[3], time = t))
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        foodT = []
        for x, y in food:
            foodT.append(PropSymbolExpr(food_str, x, y, time = t))
        model = findModel(~disjoin(foodT) & conjoin(KB))
        if (model):
            return extractActionSequence(model, actions)

        moves = [PropSymbolExpr(move, time = t) for move in actions]
        KB.append(exactlyOne(moves))

        for x, y in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(x, y, t + 1, walls))
        for x, y in food:
            first = PropSymbolExpr(pacman_str, x, y, time = t) 
            second = ~PropSymbolExpr(food_str, x, y, time = t)
            third = ~PropSymbolExpr(food_str, x, y, time = t + 1)
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for (x, y) in non_outer_wall_coords:
            ...
            if not findModel(conjoin(KB) & ~pac):
                KB.append(pac)
            if not findModel(conjoin(KB) & pac):
                KB.append(~pac)
            if findModel(conjoin(KB) & pac):
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        #KB.append(SLAMSuccessorAxioms(t + 1, known_map, non_outer_wall_coords))
        percepts = agent.getPercepts()
        action_t = agent.actions[t]
        KB.append(PropSymbolExpr(action_t, time = t))
